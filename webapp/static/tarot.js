...
(() => {
  ...
  form.addEventListener("submit", async event => {
    ...
    try {
      ...
      const spreadCards = element("div", undefined, "tarot-spread");
      data.cards.forEach((card, index) => {
        ...
        article.append(face, element("h4", card.question));
        const details = element("details");
        details.open = true;
        details.append(element("summary", "Read this card"));
        card.paragraphs.forEach(text => details.append(element("p", text)));
        article.append(details);
        spreadCards.append(article);
      });
      fragment.append(spreadCards, element("h3", "Read the spread together"), element("p", data.synthesis));
