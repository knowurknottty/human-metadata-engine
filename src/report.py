...
This run also includes **{len(extensions)} provenance-aware extensions**. Each names its own source convention, epistemic class, and input limits in the API/dashboard. Those extensions are deliberately **excluded from the composite resonance score**: historical reference data and symbolic correspondences must never be presented as measured identity signals.
...
**Extension boundaries.** The expansion layer contains provenance-aware extensions spanning historic number systems, historical-textual corpora, writing traditions, comparative correspondences, and structural Unicode analysis. Historical-textual reference systems, including the Sumerian *me* ontology, remain identity-independent by default: the presence of an item in an ancient corpus is not evidence that a modern person possesses or corresponds to it. Every one names the versioned convention used, preserves the input script, and records whether it used a native mapping, the built-in transliteration profile, or only a structural representation. It is intentionally kept outside the composite resonance score and fingerprint: adding more traditions must not create the appearance of more empirical evidence. Some systems expose only a partial calculation because the necessary primary inputs are absent. For example, full Bazi and Jyotish calculations require a precise birth time, timezone, location, and appropriate ephemeris; character stroke counts require a sourced dictionary; and cuneiform, Egyptian, and Indus inputs are not assigned invented readings. In those cases, the result says what it did compute and what it could not compute. The provenance catalog is a review trail for conventions, not a claim that any one convention is uniquely authoritative.
