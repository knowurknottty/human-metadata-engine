...
def generate_narrative(sig: dict, identity_name: str = "") -> str:
    ...
    # Header
    sections.append(f"# {name} — Personality Profile\n")
    ...
    if polarity or consonant_power:
        ...
        if total > 0:
            ...
            if v_ratio > 0.6:
                sections.append(f"The name leans feminine/creative in polarity ({v_ratio:.0%} vowel energy).")
            elif v_ratio < 0.4:
                sections.append(f"The name leans masculine/analytical in polarity ({v_ratio:.0%} vowel energy).")
            else:
    ...
    # Conclusion
    sections.append("---")
    sections.append(f"*Generated by the Human Metadata Engine v0.4.0 — personality narrative from deterministic encoding.*")
